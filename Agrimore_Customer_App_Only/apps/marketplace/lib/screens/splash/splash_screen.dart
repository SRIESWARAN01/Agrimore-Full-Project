...
class _SplashScreenState extends State<SplashScreen>
  ...
  void initState() {
    ...
    // Skip splash animation on web - show landing page instead
    if (kIsWeb) {
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        ...
        try {
          ...
          if (authProvider.isLoggedIn) {
            ...
            // Show landing page for non-logged-in web users
            Navigator.of(context).pushReplacementNamed('/landing');
          }
        } catch (e) {
          debugPrint('❌ Web splash navigation error: $e');
          if (mounted) {
            Navigator.of(context).pushReplacementNamed('/landing');
          }
        }
      });
      // Initialize controllers but don't animate
    ...
    // Quick navigation - reduced duration for mobile
  ...
  void _initializeControllers() {
    ...
    if (!kIsWeb) _glowController.repeat(reverse: true);

    _rotateController = AnimationController(
      vsync: this,
      duration: const Duration(seconds: 15), // Faster rotation
    );
    if (!kIsWeb) _rotateController.repeat();
  ...
  Future<void> _navigateToNext() async {
    ...
    try {
      // ✅ FIXED: Wait for Firebase auth to restore session before navigating
      final authProvider = Provider.of<app_auth.AuthProvider>(context, listen: false);
      ...
      debugPrint('🔐 Auth state: isLoggedIn=${authProvider.isLoggedIn}, isInitializing=${authProvider.isInitializing}');
      debugPrint('🚀 Navigating to: /main (auth restored)');

      debugPrint('🚀 Navigating to: /main (auth restored)');

      if (mounted) {
        // Fix for !_debugLocked error: Using SchedulerBinding.instance.addPostFrameCallback
        // ensures we are strictly NOT in a build phase when we navigate.
