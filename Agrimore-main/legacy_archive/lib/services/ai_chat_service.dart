...
import 'package:agroconnect/config/gemini_config.dart';
  ...
  Future<Map<String, dynamic>> handleProductSearch(String query, {String? categoryId, int limit = 10}) async {
    try {
      ...
      if (query.trim().isNotEmpty) {
        ...
        final results = await queryRef
            .where('searchKeywords', arrayContains: lowerQuery)
            .limit(limit)
            .get();

        final products = results.docs.map((d) {
        ...
        final popularResults = await queryRef.where('isFeatured', isEqualTo: true).limit(limit).get();
        final products = popularResults.docs.map((d) {
          final data = d.data() as Map<String, dynamic>;
          return {
            'id': d.id,
            'name': data['name'] ?? 'Unnamed',
